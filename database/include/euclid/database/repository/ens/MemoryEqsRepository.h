...
#include <euclid/database/repository/ens/IEqsRepository.h>
...
namespace Euclid::Database {
     ...
     */
    class MemoryEnsRepository final : public IEqsRepository {
    ...
    public:
        ...
        static MemoryEnsRepository &instance() {
            static MemoryEnsRepository sqsDatabase;
            return sqsDatabase;
        }

        Entity::ENS::Topic upsertTopic(Entity::ENS::Topic &queue) override {
            std::lock_guard lock(_mutex);
            if (queue.oid.empty()) {
                queue.oid = Core::UuidUtils::CreateRandomUuid();
                _topicStore[queue.oid] = queue;
            }
            return queue;
        }
