...
namespace Euclid::Database {
    ...
    /**
     * @brief EKM memory database.
     *
     * @author jens.vogt\@opitz-consulting.com
     */
    class MemoryEkmRepository final : public IEkmRepository {
    ...
    public:
         ...
         */
        static MemoryEkmRepository &instance() {
            static MemoryEkmRepository ekmDatabase;
            return ekmDatabase;
        }
        ...
        Entity::EKM::Key upsertKey(Entity::EKM::Key &key) override {
            ...
            if (key.oid.empty()) {
                key.oid = Core::UuidUtils::CreateRandomUuid();
                _keyStore[key.oid] = key;
            }
            return key;
        }
        ...
        std::vector<Entity::EKM::Key> listKeys(const std::string &accountId, const std::string &namespaceName, const std::string &prefix, const long pageSize, const long pageIndex, const std::string &sortColumn) const override {
            std::lock_guard lock(_mutex);
            ...
            if (sortColumn == "name") {
                std::ranges::sort(result, {}, &Entity::EKM::Key::name);
            } else if (sortColumn == "ern") {
                std::ranges::sort(result, {}, &Entity::EKM::Key::ern);
            }
        ...
        long countKeys(const std::string &accountId, const std::string &namespaceName) const override {
            std::lock_guard lock(_mutex);
        ...
        // }
        //
        // void upsertMessage(const Entity::EKM::Message &message) override {
        //     std::lock_guard lock(_mutex);
        //     // TODO: fix me
        //     //_messageStore[message.name] = message;
        // }
        //
        // Entity::EKM::Message sendMessage(const std::string &messageId, const std::string &ern, const std::string &queueErn, const std::string &body, const std::map<std::string, Entity::COM::Variant> &attributes, const Entity::EKM::MessagePriority priority) override {
        //     std::lock_guard lock(_mutex);
        //
        //     Entity::EKM::Message message;
        //     message.ern = ern;
        ...
        //     message.md5Attributes = Entity::EKM::Message::ComputeAttributesMd5(attributes);
        ...
        //             if (queue.delay > 0) {
        //                 message.status = Entity::EKM::MessageStatus::DELAYED;
        ...
        // }
        //
        // std::vector<Entity::EKM::Message> receiveMessages(const std::string &queueErn, const long maxCount, const long waitTime) override {
        //     const auto deadline = std::chrono::steady_clock::now() + std::chrono::seconds(waitTime);
        //     const auto weights = Entity::EKM::LoadPriorityWeights();
        //     static constexpr std::array priorityOrder{Entity::EKM::MessagePriority::HIGH, Entity::EKM::MessagePriority::MIDDLE, Entity::EKM::MessagePriority::LOW};
        //
        //     while (true) {
        //         {
        //             std::lock_guard lock(_mutex);
        ...
        //             }
        //
        //             std::map<Entity::EKM::MessagePriority, long> availableCounts;
        //             for (const auto &message: _messageStore | std::views::values) {
        //                 if (message.queueErn == queueErn && message.status == Entity::EKM::MessageStatus::AVAILABLE) {
        //                     availableCounts[message.priority] += 1;
        //                 }
        //             }
        //             const auto takeCounts = Entity::EKM::ComputeReceiveCounts(maxCount, availableCounts, weights);
        //
        //             std::vector<Entity::EKM::Message> result;
        //             long movedCount = 0;
        //             long movedSize = 0;
        ...
        //                     if (message.queueErn != queueErn || message.status != Entity::EKM::MessageStatus::AVAILABLE || message.priority != priority) continue;
        //
        ...
        //                         message.status = Entity::EKM::MessageStatus::AVAILABLE;
        ...
        //                     }
        //
        //                     message.status = Entity::EKM::MessageStatus::INVISIBLE;
        ...
        //             queue.size -= message.size;
        //             if (message.status == Entity::EKM::MessageStatus::AVAILABLE) {
        //                 queue.available -= 1;
        //             } else if (message.status == Entity::EKM::MessageStatus::DELAYED) {
        //                 queue.delayed -= 1;
        //             } else if (message.status == Entity::EKM::MessageStatus::INVISIBLE) {
        //                 queue.invisible -= 1;
        //             }
        //             queue.modified = std::chrono::system_clock::now();
        //             break;
        ...
        // }
        //
        // std::optional<Entity::EKM::Message> findMessageByName(const std::string &name) const override {
        //     std::lock_guard lock(_mutex);
        ...
        // }
        //
        // std::optional<Entity::EKM::Message> findMessageById(const std::string &id) const override {
        //     std::lock_guard lock(_mutex);
        ...
        // }
        //
        // std::vector<Entity::EKM::Message> findAllMessages() const override {
        //     std::lock_guard lock(_mutex);
        //     std::vector<Entity::EKM::Message> result;
        ...
        // }
        //
        // std::vector<Entity::EKM::Message> listMessages(const std::string &queueErn, const long pageSize, const long pageIndex, const std::string &sortColumn) const override {
        //     std::lock_guard lock(_mutex);
        //     std::vector<Entity::EKM::Message> result;
        //     for (const auto &m: _messageStore | std::views::values) {
        ...
        //         std::ranges::sort(result, {}, &Entity::EKM::Message::created);
        //     } else if (sortColumn == "size") {
        //         std::ranges::sort(result, {}, &Entity::EKM::Message::size);
        //     } else if (sortColumn == "messageId") {
        //         std::ranges::sort(result, {}, &Entity::EKM::Message::messageId);
        //     }
        //
        //     if (pageSize > 0) {
        //         const auto offset = std::min<size_t>(std::max<long>(pageIndex, 0) * pageSize, result.size());
        ...
        //
        //     for (auto &message: _messageStore | std::views::values) {
        //         if (message.status == Entity::EKM::MessageStatus::INVISIBLE) {
