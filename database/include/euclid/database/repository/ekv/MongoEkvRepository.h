...
namespace Euclid::Database {
    ...
    public:
        ...
        [[nodiscard]]
        bool tableExists(const std::string &accountId, const std::string &name) const override;

        [[nodiscard]]
        std::optional<Entity::EKV::Table> findTable(const std::string &accountId, const std::string &name) const override;

        [[nodiscard]]
        std::vector<Entity::EKV::Table> listTables(const std::string &accountId, const std::string &prefix, long pageSize,
                                                   long pageIndex, const std::string &sortColumn,
                                                   const std::string &sortDirection) const override;

        [[nodiscard]]
        long countTables(const std::string &accountId) const override;

        long deleteTable(const std::string &accountId, const std::string &name) override;

        Entity::EKV::Item putItem(Entity::EKV::Item &item) override;

        [[nodiscard]]
        std::optional<Entity::EKV::Item> getItem(const std::string &accountId, const std::string &tableName,
                                                 const Entity::EKV::Value &partitionKey,
                                                 const std::optional<Entity::EKV::Value> &sortKey) const override;

        bool deleteItem(const std::string &accountId, const std::string &tableName,
                        const Entity::EKV::Value &partitionKey,
                        const std::optional<Entity::EKV::Value> &sortKey) override;

        [[nodiscard]]
        std::vector<Entity::EKV::Item> query(const std::string &accountId, const std::string &tableName,
                                             const Entity::EKV::Value &partitionKey,
                                             const Entity::EKV::SortCondition &condition, bool forward,
                                             long pageSize, long pageIndex) const override;

        [[nodiscard]]
        std::vector<Entity::EKV::Item> scan(const std::string &accountId, const std::string &tableName,
                                            long pageSize, long pageIndex) const override;

        [[nodiscard]]
        long countItems(const std::string &accountId, const std::string &tableName) const override;

    private:

        /**
         * @brief Creates the indexes the store is unusable without: one table per name per
         * account, one item per key per table.
         */
         ...
         */
        [[nodiscard]]
        static bsoncxx::document::value keyFilter(const std::string &accountId, const std::string &tableName,
                                                  const Entity::EKV::Value &partitionKey,
