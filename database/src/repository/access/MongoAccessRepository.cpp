...
namespace Euclid::Database {
    ...
    Entity::Access::User MongoAccessRepository::upsertUser(Entity::Access::User &user) {
        ...
        try {
            ...
            if (auto result = userCollection.find_one_and_update(filter.view(), update.view(), opts)) {
                return Entity::Access::User::fromDocument(result->view());
            }
    ...
    std::optional<Entity::Access::User> MongoAccessRepository::findUserByUserId(const std::string &userId) const {
        ...
        try {
            ...
            if (auto result = userCollection.find_one(make_document(kvp("userId", userId)))) {
                return Entity::Access::User::fromDocument(result.value());
            }
    ...
    std::optional<Entity::Access::User> MongoAccessRepository::findUserByEmail(const std::string &email) const {
    ...
    std::optional<Entity::Access::User> MongoAccessRepository::findUserByAccessKeyId(const std::string &accessKeyId) const {
    ...
    bool MongoAccessRepository::userExists(const std::string &userId) const {
    ...
    long MongoAccessRepository::countUsers() const {
    ...
    std::vector<Entity::Access::User> MongoAccessRepository::listUsers(const std::string &prefix, const long pageSize, const long pageIndex, const std::string &sortColumn) const {
        ...
        try {
            ...
            for (auto cursor = userCollection.find(filter.view(), opts); auto doc: cursor) {
                users.push_back(Entity::Access::User::fromDocument(doc));
            }
    ...
    void MongoAccessRepository::deleteUser(const std::string &userId) const {
