...
namespace Euclid::Database::Entity {
    ...
    namespace {
        ...
        // The same leniency for the other types, and for the same reason getBsonInt() has it: a
        // module document is read on every reconcile, and the reader used to ask an element for a
        // string without asking what it was first. One field of one document with an unexpected
        // type - a null where a name should be, a number where a path should be - threw out of the
        // whole loop, findAll() caught it and answered with no modules at all, and the manager
        // read that as an installation with nothing running.
        //
        // A field euclid cannot make sense of is skipped and the rest of the document is read.
        ...
        bool getBsonBool(const bsoncxx::document::element &field, const bool fallback = false) {
            if (field.type() == bsoncxx::type::k_bool) return field.get_bool().value;
            return fallback;
        }

        std::chrono::system_clock::time_point getBsonDate(const bsoncxx::document::element &field) {
            if (field.type() == bsoncxx::type::k_date) return std::chrono::system_clock::time_point{field.get_date().value};
            return {};
        }
    }// namespace
    ...
    ModuleInstance ModuleInstance::fromDocument(const bsoncxx::document::view &doc) {
        ...
        for (const auto &field: doc) {
            ...
            // Written by the instance itself, never by the manager - see the entity. A double on
            // the way in, but an installation that stored a whole number reads back as int32/64.
            else if (key == "utilisation") instance.utilisation = field.type() == bsoncxx::type::k_double ? field.get_double().value : static_cast<double>(getBsonInt(field));
            else if (key == "backlog") instance.backlog = getBsonInt(field);
            else if (key == "loadReportedAt") instance.loadReportedAt = getBsonDate(field);
            else if (key == "created") instance.created = getBsonDate(field);
            else if (key == "modified") instance.modified = getBsonDate(field);
        }
        return instance;
    }
    ...
    Module Module::fromDocument(const std::optional<bsoncxx::document::view> &doc) {
        ...
        Module module;
        for (const auto &field: *doc) {
            if (const auto key = field.key(); key == "name") module.name = getBsonString(field);
            else if (key == "executable") module.executable = getBsonString(field);
            else if (key == "socketPath") module.socketPath = getBsonString(field);
            else if (key == "active") module.active = getBsonBool(field);
            else if (key == "autoRestart") module.autoRestart = getBsonBool(field);
            ...
            else if (key == "logLevel") module.logLevel = getBsonString(field);
            else if (key == "core") module.core = getBsonBool(field);
            else if (key == "desiredStopped") module.desiredStopped = getBsonBool(field);
            else if (key == "restartRequestedAt") module.restartRequestedAt = getBsonDate(field);
            else if (key == "args") {
                if (field.type() != bsoncxx::type::k_array) continue;
                for (const auto &elem: field.get_array().value) {
                    if (elem.type() == bsoncxx::type::k_string) module.args.emplace_back(elem.get_string().value);
                }
            } else if (key == "instances") {
                if (field.type() != bsoncxx::type::k_array) continue;
                for (const auto &elem: field.get_array().value) {
                    // An entry that is not a document is not an instance; skipping it loses that
                    // one rather than every module in the installation.
                    if (elem.type() == bsoncxx::type::k_document) module.instances.push_back(ModuleInstance::fromDocument(elem.get_document().value));
                }
            } else if (key == "created") module.created = getBsonDate(field);
            else if (key == "modified") module.modified = getBsonDate(field);
            else if (key == "lastStartTime") module.lastStartTime = getBsonDate(field);
            else if (key == "_id") module.oid = field.type() == bsoncxx::type::k_oid ? field.get_oid().value.to_string() : std::string();
        }
        return module;
    }
