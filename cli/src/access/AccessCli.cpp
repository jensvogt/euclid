...
namespace Euclid::CLI {
    ...
    AccessCli::AccessCli(std::string endpoint, Credentials::Entry authentication, const bool pretty, std::string caCertPath) : _endpoint(std::move(endpoint)), _authentication(std::move(authentication)), _pretty(pretty), _caCertPath(std::move(caCertPath)) {}

    int AccessCli::process(const std::string &action, const std::vector<std::string> &args) const {
        if (action == "help" || action == "--help" || action == "-h") {
            return PrintModuleHelp("access", {
    ...
    int AccessCli::login(const std::vector<std::string> &args) const {
        po::options_description desc("access login options");
        desc.add_options()
        ...
        Dto::Access::LoginRequest request;
        ...
        try {
            ...
            const HttpResponse response = client.Post("access", "login", boost::json::value_from(request));
            ...
            const auto loginResponse = boost::json::value_to<Dto::Access::LoginResponse>(response.body);
    ...
    int AccessCli::registerUser(const std::vector<std::string> &args) const {
        po::options_description desc("register options");
        desc.add_options()
        ...
        if (IsHelpRequest(args)) {
            return PrintActionHelp("access", "register", "--user <username> --email <email> --password <password> --region <region> --account-id <account-id>",
        ...
        Dto::Access::RegisterRequest request;
        ...
        try {
            ...
            const HttpResponse response = client.Post("access", "register", boost::json::value_from(request));
    ...
    int AccessCli::listUsers(const std::vector<std::string> &args) const {
        po::options_description desc("list users");
        desc.add_options()
        ...
        if (IsHelpRequest(args)) {
            return PrintActionHelp("access", "list-users", "[--prefix <prefix>] [--pageSize <n>] [--pageIndex <n>] [--sortColumn <column>]",
        ...
        Dto::Access::ListUserRequest request;
    ...
    int AccessCli::deleteUser(const std::vector<std::string> &args) const {
        po::options_description desc("delete user");
        desc.add_options()
                ("userId,u", po::value<std::string>()->required(), "user ID");

        if (IsHelpRequest(args)) {
            return PrintActionHelp("access", "delete-user", "--userId <userId>",
        ...
        Dto::Access::DeleteUserRequest request;
        request.userId = vm["userId"].as<std::string>();

        try {
            const HttpClient client(_endpoint, _authentication, _caCertPath);

            if (const HttpResponse response = client.Post("access", "delete-user", boost::json::value_from(request)); !response.IsSuccess()) {
    ...
    int AccessCli::createAccessKey(const std::vector<std::string> &args) const {
        if (IsHelpRequest(args)) {
            return PrintActionHelp("access", "create-access-key", "",
        ...
        try {
            ...
            const auto created = boost::json::value_to<Dto::Access::CreateAccessKeyResponse>(response.body);
    ...
    int AccessCli::listAccessKeys(const std::vector<std::string> &args) const {
        if (IsHelpRequest(args)) {
            return PrintActionHelp("access", "list-access-keys", "",
    ...
    int AccessCli::deleteAccessKey(const std::vector<std::string> &args) const {
        po::options_description desc("delete access key");
        desc.add_options()
                ("accessKeyId,k", po::value<std::string>()->required(), "access key ID");

        if (IsHelpRequest(args)) {
            return PrintActionHelp("access", "delete-access-key", "--accessKeyId <accessKeyId>",
        ...
        Dto::Access::DeleteAccessKeyRequest request;
        request.accessKeyId = vm["accessKeyId"].as<std::string>();

        try {
            const HttpClient client(_endpoint, _authentication, _caCertPath);

            if (const HttpResponse response = client.Post("access", "delete-access-key", boost::json::value_from(request)); !response.IsSuccess()) {
