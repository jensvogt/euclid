...
namespace Euclid::CLI {
    ...
    int EqsCli::createQueue(const std::vector<std::string> &args) const {
        ...
        if (IsHelpRequest(args)) {
            return PrintActionHelp("eqs", "create-queue", "--name <name> [--visibility <seconds>] [--max-retries <value>] [--max-length <value>] [dlq-name <name>] [--delay <seconds>] "
                                   ...
                                   "visibility defaults to 30 seconds, max retries defaults to 3, delay to 0, and max message length defaults to 1MB. If the default priority is given all messages are getting this"
                                   "as default priority. The default priority can by overwritten by the individual send-message command.",
                                   desc);
        }
    ...
    int EqsCli::setMessageAttribute(const std::vector<std::string> &args) const {
        ...
        desc.add_options()
                ...
                ("type,t", po::value<std::string>()->required(), "attribute type");

        if (IsHelpRequest(args)) {
            return PrintActionHelp("eqs", "set-message-attribute", "--message-id <messageId> --key <key> --value <value> --type <type>",
        ...
        Dto::EQS::SetMessageAttributeRequest request;
        request.messageId = vm["message-id"].as<std::string>();
    ...
    int EqsCli::getMessageMetadata(const std::vector<std::string> &args) const {
        ...
        if (IsHelpRequest(args)) {
            return PrintActionHelp("eqs", "get-message-metadata", "--message-id <messageId>",
                                   "Shows a message's metadata: messageId, queueErn, receiptHandle, status, priority, size (bytes), receivedCount, visibilityTimeout, contentType, created, modified.",
                                   desc);
        }
