...
namespace Euclid::CLI {
    ...
    int StorageCli::deleteBucket(const std::vector<std::string> &args) const {
        ...
        Dto::Storage::DeleteBucketRequest request;
        request.ern = vm["ern"].as<std::string>();

        try {
            const HttpClient client(_endpoint, _authentication, _caCertPath);
            if (const HttpResponse response = client.Post("storage", "delete-bucket", boost::json::value_from(request)); !response.IsSuccess()) {
    ...
    int StorageCli::listBuckets(const std::vector<std::string> &args) const {
        ...
        Dto::Storage::ListBucketsRequest request;
        ...
        try {
            const HttpClient client(_endpoint, _authentication, _caCertPath);
            const HttpResponse response = client.Post("storage", "list-buckets", boost::json::value_from(request));
            if (!response.IsSuccess()) {
    ...
    std::optional<std::string> StorageCli::createUpload(const std::string &bucketErn, const std::string &key, const int concurrency) const {
        ...
        try {
            ...
            return boost::json::value_to<Dto::Storage::CreateUploadResponse>(response.body).uploadId;
        } catch (const std::exception &ex) {
            std::cerr << "error: " << ex.what() << std::endl;
            return std::nullopt;
        }
    ...
    std::optional<boost::json::value> StorageCli::completeUpload(const std::string &uploadId) const {
        Dto::Storage::CompleteUploadRequest request;
        request.uploadId = uploadId;

        try {
            const HttpClient client(_endpoint, _authentication, _caCertPath);
            const HttpResponse response = client.Post("storage", "complete-upload", boost::json::value_from(request));
            if (!response.IsSuccess()) {
