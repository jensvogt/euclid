...
namespace Euclid::CLI {
    ...
    public:
        ...
        /**
         * @brief Whether a bucket exists, answered as an exit code a shell can branch on.
         *
         * @par
         * Written for `if euclid-cli esm exists-bucket -b name; then`. 0 when it is there, 1 when it
         ...
         * @return 0 if the bucket exists, 1 if it does not, 2 if the question could not be answered
         */
        [[nodiscard]]
        int existsBucket(const std::vector<std::string> &args) const;
