...
namespace Euclid::Dto::ESM {
    ...
    /**
     * @brief One bucket, as list-buckets describes each of its own.
     *
     * @par
     * The same Bucket model rather than a shape of its own, so that what a listing shows and what
     * this shows cannot drift apart - a field added to one is in the other by construction.
     */
    struct GetBucketResponse : BaseDto {
