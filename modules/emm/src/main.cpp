...
#define DEFAULT_SOCKET_PATH        "C:\\Program Files\\euclid\\data\\run\\euclid-emm.sock"
#else
#define DEFAULT_CONFIGURATION_FILE "/usr/local/euclid/etc/euclid.json"
#define DEFAULT_SOCKET_PATH        "/var/run/euclid-emm.sock"
#endif
...
static std::optional<CliOptions> parseCommandLine(int argc, char *argv[]) {
    ...
    po::options_description all("EMM options");
    ...
    try {
        ...
        if (vm.contains("help")) {
            std::cout << "EMM v" << APP_VERSION << " - EMM service process\n\n" << all << "\n";
            return std::nullopt;
        }

        if (vm.contains("version")) {
            std::cout << "EMM version " << APP_VERSION << "\n";
            return std::nullopt;
        }
...
int main(const int argc, char *argv[]) {
    ...
    Euclid::Core::LogStream::SetProcessChannel("emm");
    ...
    // Audited like any other module, and deliberately so despite emm being unbindable: role
    // binding is about who may call these, and the audit is about what was called. restart-module
    // and stop-module are exactly the commands somebody reviewing an incident is looking for.
    Euclid::Database::WireAuditSink();

    Euclid::Core::Monitoring::MetricsPusher metricsPusher("emm");
    try {
        Euclid::EMM::EmmServer server(cliOpts->socketPath,
                                      Euclid::Core::HttpActionServer::ConfiguredWorkerThreads("emm", 2));
        return server.RunUntilSignal();
    } catch (const std::exception &e) {
        log_error << "Failed to start EMM service: " << e.what();
        return 1;
    } catch (...) {
        log_error << "Failed to start EMM service: unknown exception type";
        return 1;
    }
}