...
#define DEFAULT_SOCKET_PATH        "/var/run/euclid/euclid-acc.sock"
...
static std::optional<CliOptions> parseCommandLine(int argc, char *argv[]) {
    ...
    po::options_description all("euclid-acc options");
    ...
    try {
        ...
        if (vm.contains("help")) {
            std::cout << "euclid-acc v" << APP_VERSION << " - Access service process\n\n" << all << "\n";
            return std::nullopt;
        }

        if (vm.contains("version")) {
            std::cout << "euclid-acc version " << APP_VERSION << "\n";
            return std::nullopt;
        }
