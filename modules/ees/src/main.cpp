...
#define DEFAULT_SOCKET_PATH        "C:\\Program Files\\euclid\\data\\run\\euclid-ees.sock"
#else
#define DEFAULT_CONFIGURATION_FILE "/usr/local/euclid/etc/euclid.json"
#define DEFAULT_SOCKET_PATH        "/var/run/euclid-ees.sock"
#endif
...
static std::optional<CliOptions> parseCommandLine(int argc, char *argv[]) {
    ...
    po::options_description all("euclid-ees options");
    ...
    try {
        ...
        if (vm.contains("help")) {
            std::cout << "euclid-ees v" << APP_VERSION << " - Event service process\n\n" << all << "\n";
            return std::nullopt;
        }

        if (vm.contains("version")) {
            std::cout << "euclid-ees version " << APP_VERSION << "\n";
            return std::nullopt;
        }
...
int main(const int argc, char *argv[]) {
    ...
    Euclid::Core::LogStream::SetProcessChannel("ees");
