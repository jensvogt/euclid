...
#define DEFAULT_SOCKET_PATH        "C:\\Program Files\\euclid\\data\\run\\euclid-ess.sock"
#else
#define DEFAULT_CONFIGURATION_FILE "/usr/local/euclid/etc/euclid.json"
#define DEFAULT_SOCKET_PATH        "/var/run/euclid-ess.sock"
#endif
...
static std::optional<CliOptions> parseCommandLine(int argc, char *argv[]) {
    ...
    try {
        ...
        if (vm.contains("help")) {
            std::cout << "ESS v" << APP_VERSION << " - ESS service process\n\n" << all << "\n";
            return std::nullopt;
        }

        if (vm.contains("version")) {
            std::cout << "ESS version " << APP_VERSION << "\n";
            return std::nullopt;
        }
...
int main(const int argc, char *argv[]) {
    ...
    Euclid::Core::LogStream::SetProcessChannel("ess");
    ...
    Euclid::Core::Monitoring::MetricsPusher metricsPusher("ess");
    try {
        Euclid::ESS::EssServer server(cliOpts->socketPath,
                                      Euclid::Core::HttpActionServer::ConfiguredWorkerThreads("ess", 2));
        return server.RunUntilSignal();
    } catch (const std::exception &e) {
        log_error << "Failed to start ESS service: " << e.what();
        return 1;
    } catch (...) {
        log_error << "Failed to start ESS service: unknown exception type";
        return 1;
    }
}