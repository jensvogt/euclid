...
#define DEFAULT_SOCKET_PATH        "C:\\Program Files\\euclid\\data\\run\\euclid-eag.sock"
#else
#define DEFAULT_CONFIGURATION_FILE "/usr/local/euclid/etc/euclid.json"
#define DEFAULT_SOCKET_PATH        "/var/run/euclid-eag.sock"
#endif
...
static std::optional<CliOptions> parseCommandLine(int argc, char *argv[]) {
    ...
    po::options_description all("euclid-eag options");
    ...
    try {
        ...
        if (vm.contains("help")) {
            std::cout << "euclid-eag v" << APP_VERSION << " - API gateway process\n\n" << all << "\n";
            return std::nullopt;
        }

        if (vm.contains("version")) {
            std::cout << "euclid-eag version " << APP_VERSION << "\n";
            return std::nullopt;
        }
...
int main(const int argc, char *argv[]) {
    ...
    Euclid::Core::Monitoring::MetricsPusher metricsPusher("eag");
    try {
        Euclid::EAG::EagServer server(cliOpts->socketPath, Euclid::Core::HttpActionServer::ConfiguredWorkerThreads("eag", 8));
        return server.RunUntilSignal();
    } catch (const std::exception &e) {
        log_error << "Failed to start application service: " << e.what();
        return 1;
    } catch (...) {
        log_error << "Failed to start application service: unknown exception type";
        return 1;
    }
}