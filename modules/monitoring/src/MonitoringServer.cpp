...
namespace Euclid::Monitoring {
    ...
    namespace {
        // Looks up the caller identity resolved by MonitoringServer::Authenticate(), by user ID.
        struct AuthResult {
        ...
        // its own push tick), drained by MonitoringServer::flush(). There's only ever one
        // MonitoringServer per process, so file-local state (rather than instance members reached
    ...
    static response<string_body> unauthorized(const request<string_body> &req, const AuthResult &auth) {
        return MonitoringServer::Unauthorized(req, {.subject = std::nullopt, .tokenExpired = auth.tokenExpired, .denialReason = auth.denialReason});
    }
    ...
    static response<string_body> handlePushMetrics(const request<string_body> &req) {
        ...
        boost::json::value jv;
        if (const auto err = MonitoringServer::ParseJsonBody(req, jv)) return *err;
        if (!jv.is_object()) return MonitoringServer::ErrorResponse(req, status::bad_request, "Expected a JSON object body");
        ...
        if (!moduleName || !moduleName->is_string() || !items || !items->is_array()) {
            return MonitoringServer::ErrorResponse(req, status::bad_request, "Expected \"module\" and \"items\"");
        }
        ...
        return MonitoringServer::JsonResponse(req, status::ok);
    }

    // ── "list" action handler ────────────────────────────────────────────────
    // Admin-only, same pattern as AccessServer's handleListUsers - otherwise the data this module
    ...
    static response<string_body> handleList(const request<string_body> &req) {
        ...
        if (!auth.user->isAdmin) {
            return MonitoringServer::ErrorResponse(req, status::forbidden, "Administrator privileges required");
        }
        ...
        const auto rows = Database::RepositoryFactory::instance().monitoringRepository()->list(name, labelName, labelValue, limit);
        ...
        return MonitoringServer::JsonResponse(req, status::ok, boost::json::serialize(boost::json::object{{"items", items}}));
    }
    ...
    static response<string_body> dispatch(const request<string_body> &req) {
        ...
        if (action.empty()) {
            return MonitoringServer::ErrorResponse(req, status::bad_request, "Missing x-euclid-action header");
        }
        ...
        return MonitoringServer::ErrorResponse(req, status::not_found, "Action not implemented: " + action);
    }

    // ── MonitoringServer ─────────────────────────────────────────────────────

    MonitoringServer::MonitoringServer(std::string socketPath, const int threads) : HttpActionServer("Monitoring", std::move(socketPath), threads) {
    ...
    void MonitoringServer::flush() {
        ...
        const auto repo = Database::RepositoryFactory::instance().monitoringRepository();
    ...
    void MonitoringServer::prune() {
        ...
        Database::RepositoryFactory::instance().monitoringRepository()->deleteOlderThan(cutoff);
    }

    response<string_body> MonitoringServer::Dispatch(const request<string_body> &req) {
        return dispatch(req);
    }
