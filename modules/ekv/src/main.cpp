...
#define DEFAULT_SOCKET_PATH        "C:\\Program Files\\euclid\\data\\run\\euclid-ekv.sock"
#else
#define DEFAULT_CONFIGURATION_FILE "/usr/local/euclid/etc/euclid.json"
#define DEFAULT_SOCKET_PATH        "/var/run/euclid-ekv.sock"
#endif
...
static std::optional<CliOptions> parseCommandLine(int argc, char *argv[]) {
    ...
    po::options_description all("ESS options");
    ...
    try {
        ...
        if (vm.contains("help")) {
            std::cout << "EKV v" << APP_VERSION << " - EKV service process\n\n" << all << "\n";
            return std::nullopt;
        }

        if (vm.contains("version")) {
            std::cout << "EKV version " << APP_VERSION << "\n";
            return std::nullopt;
        }
...
int main(const int argc, char *argv[]) {
    ...
    Euclid::Core::LogStream::SetProcessChannel("ekv");
    ...
    // is refused on resource grounds at all.
    Euclid::Database::WireResourceLookup();

    Euclid::Core::Monitoring::MetricsPusher metricsPusher("ekv");
    try {
        Euclid::EKV::EkvServer server(cliOpts->socketPath,
                                      Euclid::Core::HttpActionServer::ConfiguredWorkerThreads("ekv", 2));
        return server.RunUntilSignal();
    } catch (const std::exception &e) {
        log_error << "Failed to start EKV service: " << e.what();
        return 1;
    } catch (...) {
        log_error << "Failed to start EKV service: unknown exception type";
        return 1;
    }
}