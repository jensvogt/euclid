...
Usage: send_test_messages.sh [-n COUNT] [-q QUEUE_NAME] [-e ENDPOINT] [-c CLI_PATH] [-r]
  ...
  -n COUNT       number of messages to send (default: 10)
  -q QUEUE_NAME  queue to subscribe to; created if it doesn't exist yet (default: test-queue)
...
queueErn=$(cli eqs get-queue-ern --name "$QUEUE_NAME" 2>/dev/null | jq -r '.ern // empty')
if [ -z "$queueErn" ]; then
    echo "Queue '$QUEUE_NAME' not found, creating it..."
    queueErn=$(cli eqs create-queue --name "$QUEUE_NAME" | jq -r '.ern')
else
  echo "Purging queue '$queueErn'"
  cli eqs purge-queue --queue "$queueErn"
fi

# Resolve the topic's ERN, creating the topic on the fly if it doesn't exist yet.
topicErn=$(cli ens get-topic-ern --name "$TOPIC_NAME" 2>/dev/null | jq -r '.ern // empty')
if [ -z "$topicErn" ]; then
    echo "Topic '$TOPIC_NAME' not found, creating it..."
    topicErn=$(cli ens create-topic --name "$TOPIC_NAME" | jq -r '.ern')
else
  echo "Purging topic '$topicErn'"
  cli ens purge-topic --topic "$TOPIC_NAME"
fi
subscriptionErn=$(cli ens subscribe --source "$topicErn" --target "$queueErn" --type SQS 2>/dev/null | jq -r '.ern // empty')
echo "Subscribed '$queueErn' to topic '$topicErn', subscription: '$subscriptionErn'"

echo "Publish $COUNT messages to '$TOPIC_NAME' (ern: $topicErn)"

for ((i = 1; i <= COUNT; i++)); do

    cli ens publish-message --topic "$topicErn" --body "test message $i" >/dev/null

    echo "  [$i/$COUNT] sent"
done

received=$(cli eqs get-message-count --queue "$queueErn" | jq -r '.available')

echo "Done. Received in SQS queue: $received"
